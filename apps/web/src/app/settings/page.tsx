...
export default function SettingsPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--color-bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ maxWidth: 520, width: '100%', margin: '0 auto', padding: 'var(--space-7) var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        ...
        <section style={sectionStyle}>
          <h2 style={sectionHeadingStyle}>Focus interval</h2>
          <IntervalSelector value={prefs.workIntervalMinutes} onChange={(v) => save({ workIntervalMinutes: v })} />
        </section>

        {/* Break duration */}
        <section style={sectionStyle}>
          <h2 style={sectionHeadingStyle}>Break duration</h2>
          <div style={{ width: '100%', maxWidth: 400 }}>
            <div style={{ textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 'var(--font-size-xl)', fontWeight: 'var(--font-weight-light)', color: 'var(--color-text)', marginBottom: 'var(--space-5)' }}>
        ...
        {/* Problem areas */}
        <section style={sectionStyle}>
          <h2 style={sectionHeadingStyle}>Problem areas</h2>
          <ProblemAreaPicker selected={prefs.problemAreas as ProblemArea[]} onChange={(areas) => save({ problemAreas: areas })} />
        </section>

        {/* Difficulty */}
        <section style={sectionStyle}>
          <h2 style={sectionHeadingStyle}>Exercise intensity</h2>
        ...
        {/* Theme */}
        <section style={sectionStyle}>
          <h2 style={sectionHeadingStyle}>Theme</h2>
            ...
            {THEMES.map((t) => (
              <button
                ...
                style={{
                  ...
                  transition: 'border-color 0.15s', minWidth: 72,
                }}
              >
                <div style={{ width: 28, height: 28, borderRadius: 'var(--radius-full)', backgroundColor: t.accent }} />
                <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text)', fontWeight: 'var(--font-weight-medium)' }}>
                  {t.label}
                </span>
              </button>
            ))}
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          <h2 style={sectionHeadingStyle}>Data</h2>
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            <Button variant="outline" size="sm" onClick={exportData}>Export backup</Button>
            <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}>Import backup</Button>
            <input ref={fileInputRef} type="file" accept=".json" onChange={handleImport} style={{ display: 'none' }} />
          </div>
          <div style={{ marginTop: 'var(--space-2)' }}>
            <Button variant="ghost" size="sm" onClick={handleClear}>Clear all data</Button>
          </div>
        </section>

        {saved && (
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-success)' }}>Saved.</p>
        )}
      </div>
    </main>
  );
